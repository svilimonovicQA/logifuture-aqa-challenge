describe("User Info API tests", () => {
  ...
  it("should return user information when making a GET request to /user/info/:userId", () => {
    cy.getUserInfo(userId, authToken).then((response) => {
      ...
      expect(response.body.data).to.have.property("walletId").and.not.be.empty;
      expect(response.body.data).to.have.property("name").and.not.be.empty;
      expect(response.body.data).to.have.property("locale", "en-US");
      expect(response.body.data).to.have.property("region").and.not.be.empty;
      expect(response.body.data).to.have.property("timezone").and.not.be.empty;
      expect(response.body.data).to.have.property("email").and.not.be.empty;
    });
  });
